...
Automata* Automata::read_from_stream(std::istream& in)
{
    ...
    for (int j = 0; j < transition_count; ++j) {
        in >> from >> symb >> to;
        automata->set_transition(from, symb2id[symb], to);
    }
...
void Automata::dump_to_stream(std::ofstream& out)
{
    ...
    for (int i = 0; i < this->transitions.size(); ++i)
    {
        int from = i / 4;
        int symb = i % 4;
        out << from << " " << id2symb[symb] << " " << this->transitions[i] << std::endl;
    }
}
...
Automata* sum_automata(const Automata* first_automata, const Automata* second_automata) {
    ...
    sum_automata(*first_automata, *second_automata, *result);
    return result;
}
...
void generate_automata_visualization_script(const Automata* automata, std::ostream& output)
{
    ...
    output << "    rankdir = LR;\n";
    output << "    size = \"5,5\"\n";
    output << "    node[shape = doublecircle]; S;\n";
...
void generate_automata_visualization_script(const Automata* automata, std::string filepath)
{
    ...
    generate_automata_visualization_script(automata, file);
}



static std::map<char, char> nsymb2symb = { { 'a', 'a' },{ 'c', 'c' },{ 'b', 'g' },{ 'd', 't' } };
void find_all_min_strings(const Automata* big, std::vector<int>& min_paths, int currentNode, list<string>& result) {
    result.clear();
    ...
    for (int i = 0; i < big->state_count(); ++i) {
        ...
        if (subresult.empty() == false) {
            if (min_strings.empty() == false) {
                int len_current = subresult.front().size();
                int len_before = min_strings.front().size();
                if (len_current > len_before)
                    continue;
                if (len_current < len_before)
                    min_strings.clear();
            }
