...
nucleotides = ["A", "C", "G", "T"]
mapping = {i: nucleotides[i % len(nucleotides)] for i in range(n)}
...
unique_nucleotides = set(mapping.values())

trace_nodes = []
for nucleotide in unique_nucleotides:
    indices = [i for i, n in mapping.items() if n == nucleotide]
    ...
    node_trace = go.Scatter3d(
        ...
        text=[f"Nucleotide: {nucleotide}" for _ in indices],
        hoverinfo="text",
    )
